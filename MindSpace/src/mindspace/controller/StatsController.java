...
public class StatsController implements Initializable {
    ...
    private void carregarDados() {
        ...
        for (Document doc : registos) {
            try {
                LocalDate data = LocalDate.parse(doc.getString("data"), formatter);
                String emocao = doc.getString("emotion");
                String nota = doc.getString("note");
                int pontos = doc.getInteger("points");

                entradas.add(new EmotionEntry(data, emocao, nota, pontos));
            } catch (Exception e) {
                System.err.println("Erro ao processar registo: " + doc);
                e.printStackTrace();
            }
        }
    }
